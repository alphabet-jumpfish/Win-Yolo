...
- **自动瞄准+开火功能**：
  - 仅供学习和研究使用，请勿用于非法用途
  - 在游戏中使用需要安装 `pywin32`：`pip install pywin32`
  - 开火速率可在 `ScreenDetectorService.py` 第52行调整
  - 目标范围阈值可在第56行调整（默认50像素）
...
### Q: 鼠标自动移动功能如何禁用？
**A:** 按 `K` 键关闭自动瞄准+自动开火功能，或在 `service/ScreenDetectorService.py` 中注释掉：
```python
...
### Q: 游戏中鼠标无法移动？
**A:**
1. 确保已安装 `pywin32`：`pip install pywin32`
2. 检查游戏是否有反作弊系统阻止鼠标控制
3. 尝试以管理员权限运行程序

### Q: 如何调整开火速率？
**A:** 在 `service/ScreenDetectorService.py` 第52行修改：
```python
self.fire_rate = 1  # 改为 2 表示每秒2次，改为 0.5 表示每2秒1次
```

### Q: 如何调整目标检测范围？
**A:** 在 `service/ScreenDetectorService.py` 第56行修改：
```python
self.target_range_threshold = 50  # 改为 100 更宽松，改为 30 更严格
```
